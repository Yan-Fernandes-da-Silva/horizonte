...
export function TaskItem({ planId, task }: { planId: string; task: TaskData }) {
    ...
    return (
      <div className="rounded-xl border border-white/20 bg-white/10 p-4 backdrop-blur-sm">
        ...
        <div className="mt-2 flex justify-end gap-2">
          <Button size="sm" variant="outline" onClick={() => setEditing(false)} className="border-white/30 bg-white/10 text-white hover:bg-white/20 hover:text-white">
  ...
  return (
    <div className="group flex items-start gap-3 rounded-xl border border-white/15 bg-white/10 p-4 backdrop-blur-sm">
      <button
        type="button"
        onClick={toggle}
        disabled={busy}
        ...
        className={cn(
          ...
          done ? "border-emerald-400 bg-emerald-400 text-ocean" : "border-white/40 hover:border-sky"
        )}
      >
      ...
      <div className="min-w-0 flex-1">
        <p className={cn("text-sm font-medium text-white", done && "text-white/50 line-through")}>{task.title}</p>
        {task.description && <p className="mt-0.5 text-xs text-white/70">{task.description}</p>}
      </div>

      <div className="flex shrink-0 gap-1 opacity-0 transition-opacity group-hover:opacity-100">
        <button type="button" onClick={() => setEditing(true)} aria-label="Editar" className="rounded p-1 text-white/60 hover:bg-white/10 hover:text-white">
          <Pencil className="h-4 w-4" />
        </button>
        <button type="button" onClick={() => setConfirmOpen(true)} aria-label="Excluir" className="rounded p-1 text-white/60 hover:bg-sun/10 hover:text-sun">
